...
구축한 User-Based CF, Item-Based CF, Matrix Factorization(ALS) 모델을 기반으로 생성된 Hybrid Recommendation System의 성능을 여러 지표를 이용하여 정량적으로 평가한다.

### **6.1 평가 대상 및 설정**

#### **6.1.1 평가 대상 모델**

본 프로젝트에서는 UBCF, IBCF, MF 세 가지 모델을 개발하였으나, 실제로 사용자에게 추천을 제공하는 것은 **하이브리드 모델**이므로 평가는 하이브리드 모델만을 대상으로 진행함.

* **Hybrid Model**: UBCF + IBCF + MF의 가중 앙상블
* 가중치: `w_user=0.33`, `w_item=0.33`, `w_mf=0.34`

#### **6.1.2 평가 데이터**

* **Ground Truth**: `rating_dataset.csv` (1000명 × 625조합의 완전한 평점 행렬)
* **예측 대상**: 훈련에 사용되지 않은 445개 조합

  * Top 100 인기 조합 중 20개
  * Second 200 조합 중 100개
  * Bottom 325 조합 전체
* 실제 평가는 ground truth와 예측 행렬의 공통 조합(625개)에 대해 수행

### **6.2 평가 지표**

추천 시스템의 성능을 측정하기 위해 **예측 정확도 지표**와 **추천 품질 지표**를 함께 사용함.

#### **6.2.1 예측 정확도 지표**

##### **RMSE (Root Mean Squared Error)**

* 실제 평점과 예측 평점 간 오차의 제곱근
* 큰 오차에 민감하게 반응
* 값이 작을수록 좋음

##### **MAE (Mean Absolute Error)**

* 실제 평점과 예측 평점 간 절대 오차의 평균
* RMSE보다 이상치에 덜 민감
* 값이 작을수록 좋음

#### **6.2.2 추천 품질 지표**

추천 시스템의 실질적 목표는 "사용자가 좋아할 아이템을 상위에 배치"하는 것이므로, 랭킹 기반 평가 지표가 중요함.

##### **Precision@K**

* Top-K 추천 목록 중 실제로 사용자가 선호하는 아이템의 비율
* Relevant 기준: 실제 평점 ≥ 4.0
* 본 평가에서는 K=5 사용
* 값이 클수록 추천 적중률이 높음

##### **Recall@K**

* 사용자가 선호하는 모든 아이템 중 Top-K에 포함된 비율
* K가 작으면 구조적으로 낮은 값을 가짐
* 값이 클수록 좋음

##### **NDCG@K (Normalized Discounted Cumulative Gain)**

* 추천 순서의 품질을 평가하는 지표
* 상위에 관련도 높은 아이템이 배치될수록 높은 점수
* 0~1 사이 값이며, 1에 가까울수록 이상적인 순서

### **6.3 평가 구현**

#### **6.3.1 데이터 준비**

Ground truth와 예측 행렬 간 공통 조합만 추출하여 평가를 진행함.
...
##### **(1) RMSE / MAE 계산: `calc_rmse_mae(true_df, pred_df)`**

* **입력/출력**

  * 입력: 실제 평점 DataFrame, 예측 평점 DataFrame
  * 출력: RMSE, MAE 값

* **계산 과정**

  * `mask = true_df.notna()`로 실제 평점이 존재하는 위치만 선택
  * 해당 위치에서 실제값과 예측값의 차이 계산
  * RMSE는 차이의 제곱 평균의 제곱근
  * MAE는 차이의 절댓값 평균

##### **(2) Precision@K / Recall@K 계산: `precision_recall_at_k(true_df, pred_df, k=5, threshold=4.0)`**

* **입력/출력**

  * 입력: 실제 평점, 예측 평점, k=5, 선호 기준=4.0
  * 출력: Precision@5, Recall@5

* **계산 과정**

  * 각 사용자별로 실제 평점 ≥ 4.0인 아이템을 선호 아이템으로 정의
  * 예측 점수 기준 상위 k개 아이템 추출
  * 두 집합의 교집합 크기로 hit 계산
  * Precision = hit / k
  * Recall = hit / (선호 아이템 총 개수)
  * 모든 사용자의 평균값 반환

##### **(3) NDCG@K 계산: `ndcg_at_k(true_df, pred_df, k=5)`**
...
* **DCG 계산**
  ...
  * `dcg(scores)` 함수: 각 위치의 관련도를 `(2^rel - 1) / log2(i+2)` 형태로 가중합

* **NDCG 계산 과정**

  * 예측 점수 기준 상위 k개의 실제 평점 추출 → DCG 계산
  * 실제 평점 기준 상위 k개 추출 → IDCG 계산 (이상적인 순서)
  * NDCG = DCG / IDCG
  * 모든 사용자의 평균 NDCG 반환
...
#### **7.1.1 예측 정확도 지표 (RMSE, MAE)**

**RMSE = 2.6289, MAE = 2.5024**

두 지표 모두 상당히 높은 값으로, 모델이 **정확한 평점 수치 예측에는 실패**했음을 의미함. 0~5점 범위에서 평균 2.5점 이상의 오차는 실용적 관점에서 만족스럽지 못한 수준임.

**원인 분석:**

* 사용자당 625개 조합 중 약 180개만 관측되어 있어 정보량이 제한됨
* 평점 생성 과정과 모델 구조가 일치하지 않아 절대적 평점 예측에는 불리함
* 모델이 절대적 평점보다는 상대적 순위에 집중하도록 설계됨

#### **7.1.2 추천 품질 지표 (Precision@5, Recall@5, NDCG@5)**

**Precision@5 = 0.8922**

추천된 5개 조합 중 평균 4.46개(≈89%)가 실제로 사용자가 선호하는(평점 ≥ 4.0) 아이템임. 이는 추천 시스템으로서 **매우 높은 적중률**을 의미하며, 실제 서비스 적용 시 사용자 만족도가 높을 것으로 예상됨.

**Recall@5 = 0.2435**

사용자가 선호하는 모든 아이템 중 약 24%만 Top-5에 포함됨. 이는 상대적으로 낮아 보이지만, K=5로 제한된 추천 목록에서는 **구조적으로 제한될 수밖에 없는 값**임. 사용자당 평균 선호 아이템이 20개 이상이라면 5개 안에 모두 포함시키는 것은 불가능함.

**NDCG@5 = 0.9842**

1에 매우 가까운 값으로, 모델이 **추천 순서를 거의 완벽하게** 맞추고 있음을 의미함. 가장 선호할 조합이 1위에, 그 다음이 2위에 배치되는 등 이상적인 순서와 거의 일치함.

### **7.2 지표 간 괴리 분석**

가장 주목할 점은 **예측 정확도는 낮지만(RMSE=2.63) 추천 품질은 매우 높다(NDCG=0.98)**는 상반된 결과임.

이는 모델이 **절대적 평점 수치는 부정확하게 예측**하지만, **상대적 선호도 순서는 거의 완벽하게 파악**하고 있음을 의미함.

**예시:**

* 실제 평점: 조합A(4.5점), 조합B(4.0점), 조합C(2.0점)
* 모델 예측: 조합A(3.0점), 조합B(2.8점), 조합C(1.5점)
* RMSE는 높음 (1.5점 차이)
* 하지만 순서는 정확 (A > B > C)
* 따라서 추천 품질은 우수

이러한 결과는 추천 시스템의 **실질적 목표가 정확한 평점 예측이 아니라 좋은 아이템을 상위에 배치하는 것**임을 고려할 때, 본 모델이 목적에 부합하는 성능을 보인다고 해석할 수 있음.

### **7.3 종합**

본 하이브리드 추천 시스템은:

* **회귀 문제**(평점 예측)로는 실패했으나
* **랭킹 문제**(추천 순서)로는 성공함

Precision@5 = 0.89, NDCG@5 = 0.98이라는 결과는 추천 시스템으로서 충분히 실용적인 성능을 달성했음을 보여줌.
...
| 송영우 202135546 | 아이디어 제안, 데이터셋 제작 |
| --- | --- |
| 현관 202135596 | 모델링 테스트 및 평가 |
| 황성민 202135599 | 추천 시스템 모델링 |
